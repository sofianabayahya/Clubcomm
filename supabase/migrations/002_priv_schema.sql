-- ClubComm — hulpfuncties naar een eigen schema (niet via de API aan te roepen); alleen de app-functies blijven publiek.
create schema if not exists priv;
grant usage on schema priv to authenticated;
revoke all on schema priv from anon, public;
-- ---------- Hulpfuncties (security definer: lezen zonder RLS, alleen voor de ingelogde gebruiker) ----------
create or replace function priv.cc_club() returns text language sql stable security definer set search_path = public as $$
  select club_id from public.lid where auth_uid = auth.uid() limit 1 $$;
create or replace function priv.cc_ik() returns text language sql stable security definer set search_path = public as $$
  select persoon_id from public.lid where auth_uid = auth.uid() limit 1 $$;

create or replace function priv.cc_rollen() returns setof jsonb language sql stable security definer set search_path = public as $$
  select r from public.rij p, jsonb_array_elements(coalesce(p.data->'rollen','[]'::jsonb)) r
  where p.club_id = priv.cc_club() and p.soort = 'people' and p.id = priv.cc_ik() $$;

create or replace function priv.cc_admin() returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from priv.cc_rollen() r where r->>'rol' in ('hjo','beheerder')) $$;
create or replace function priv.cc_coord() returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from priv.cc_rollen() r where r->>'rol' = 'coordinator') $$;

-- Teams waar ik staf ben: trainer/teamleider van het team, coördinator van de groep, of HJO/beheerder (alle teams)
create or replace function priv.cc_staf_teams() returns text[] language sql stable security definer set search_path = public as $$
  select coalesce(array_agg(distinct t.id), '{}') from public.rij t
  where t.club_id = priv.cc_club() and t.soort = 'teams' and (
    priv.cc_admin()
    or exists (select 1 from priv.cc_rollen() r where r->>'rol' in ('trainer','teamleider') and r->>'teamId' = t.id)
    or exists (select 1 from priv.cc_rollen() r where r->>'rol' = 'coordinator' and (r->'cats') ? (t.data->>'cat'))
  ) $$;

-- Mijn kinderen en hun teams
create or replace function priv.cc_kinderen() returns text[] language sql stable security definer set search_path = public as $$
  select coalesce(array_agg(p.id), '{}') from public.rij p
  where p.club_id = priv.cc_club() and p.soort = 'players' and (p.data->'ouders') ? priv.cc_ik() $$;
create or replace function priv.cc_kind_teams() returns text[] language sql stable security definer set search_path = public as $$
  select coalesce(array_agg(distinct p.team_id), '{}') from public.rij p
  where p.club_id = priv.cc_club() and p.soort = 'players' and (p.data->'ouders') ? priv.cc_ik() and p.team_id is not null $$;

-- Tijdelijke toegang: ik ben vervanger of begeleider bij deze activiteit
create or replace function priv.cc_act_toegang(p_act text) returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.rij a where a.club_id = priv.cc_club() and a.soort = 'acts' and a.id = p_act
    and (a.data->>'vervangerId' = priv.cc_ik() or a.data->>'begeleiderId' = priv.cc_ik())) $$;
-- Is deze persoon staf (dan mag iedereen in de club zijn contactgegevens zien)?
create or replace function priv.cc_is_staf(p_persoon text) returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.rij p, jsonb_array_elements(coalesce(p.data->'rollen','[]'::jsonb)) r
    where p.club_id = priv.cc_club() and p.soort = 'people' and p.id = p_persoon and r->>'rol' <> 'ouder') $$;
...
begin
  ...
  case p_scope
    ...
    when 'contact' then return p_persoon = ik or admin or cardinality(staf) > 0 or (not p_schrijven and priv.cc_is_staf(p_persoon));
...
create or replace function priv.cc_tijdelijk_teams() returns text[] language sql stable security definer set search_path = public as $$
  select coalesce(array_agg(distinct a.team_id), '{}') from public.rij a
  where a.club_id = priv.cc_club() and a.soort = 'acts'
    and (a.data->>'vervangerId' = priv.cc_ik() or a.data->>'begeleiderId' = priv.cc_ik())
...
create policy rij_lezen on public.rij for select to authenticated
  using (club_id = (select priv.cc_club()) and priv.cc_mag(scope, team_id, speler_id, persoon_id, act_id, data, false,
    (select priv.cc_ik()), (select priv.cc_staf_teams()), (select priv.cc_kinderen()), (select priv.cc_kind_teams()), (select priv.cc_admin()), (select priv.cc_coord()), (select priv.cc_tijdelijk_teams())));
...
create or replace function priv.cc_bewaak() returns trigger language plpgsql security definer set search_path = public as $$
declare vrij text[]; ik text := priv.cc_ik(); staf boolean;
begin
  ...
  if tg_op = 'UPDATE' and (new.scope <> old.scope or new.club_id <> old.club_id) and not priv.cc_admin() then
  ...
  if new.soort = 'people' then
  ...
  elsif new.soort = 'contact' and new.persoon_id is distinct from ik and not priv.cc_admin() then
    ...
    if tg_op = 'INSERT' and priv.cc_is_staf(new.persoon_id) then raise exception 'Contactgegevens van stafleden legt de HJO vast'; end if;
  elsif new.soort = 'acts' and tg_op = 'INSERT' and not (new.team_id = any(priv.cc_staf_teams())) then
    raise exception 'Alleen staf kan activiteiten toevoegen';
  elsif new.soort = 'msgs' and tg_op = 'UPDATE' and old.data->>'van' is distinct from ik and not priv.cc_admin() then
    vrij := array['gelezen','antw'];
  elsif new.soort = 'acts' and tg_op = 'UPDATE' then
    staf := new.team_id = any(priv.cc_staf_teams());
...
create trigger rij_bewaak before insert or update on public.rij for each row execute function priv.cc_bewaak();
...
declare ik text := priv.cc_ik(); c text := priv.cc_club();
begin
...
-- Oude publieke versies opruimen
drop function if exists public.cc_mag(text,text,text,text,text,jsonb,boolean,text,text[],text[],text[],boolean,boolean,text[]);
drop function if exists public.cc_tijdelijk_teams();
drop function if exists public.cc_is_staf(text);
drop function if exists public.cc_act_toegang(text);
drop function if exists public.cc_kind_teams();
drop function if exists public.cc_kinderen();
drop function if exists public.cc_staf_teams();
drop function if exists public.cc_coord();
drop function if exists public.cc_admin();
drop function if exists public.cc_rollen();
drop function if exists public.cc_ik();
drop function if exists public.cc_club();
drop function if exists public.cc_bewaak();
...
revoke execute on function public.koppel_mij() from public, anon;
revoke execute on function public.aanmelden(text,text,text,text,text) from public, anon;
revoke execute on function public.bericht_bij(text,boolean,jsonb) from public, anon;
