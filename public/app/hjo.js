...
(function () {
  ...
  CC.rollen.hjo = {
    context(S) { return { titel: S.club.naam, sub: `${S.club.labels.hjo} · seizoen ${(S.club.seizoen.start || "").slice(0, 4)}/${(S.club.seizoen.eind || "").slice(0, 4)}` }; },
    ...
    schermen: {
      home(S) {
        ...
        if (A.laat.length) rijen.push(h.rij({ ic: 'hourglass', titel: `${A.laat.length} aanmelding${A.laat.length > 1 ? 'en' : ''} langer dan 48 uur open`, sub: A.laat.map((x) => `${esc(x.kindVoor)} (${esc(CC.tn(x.teamId))})`).join(', '), kleur: 'oranje', act: 'open', attrs: 'data-view="aanmeldingenHjo"' }));
      ...
      teams(S) {
        ...
        if (modus === 'mensen') {
          ...
          return `${seg}<div class="zoek">${icon('search')}<input type="search" placeholder="Zoek persoon (ook ouders)" value="${esc(q)}" data-input="zoekMens" aria-label="Zoek persoon"></div>
            ...
            <div class="lijst">${res.map((p) => h.rij({ ic: h.avatar(p.naam), titel: esc(p.naam), sub: p.rollen.filter((r) => r.rol !== 'ouder' || !staf(p)).map((r) => `${esc(CC.rolNaam(r))}${r.teamId ? ' ' + esc(CC.tn(r.teamId)) : r.groep ? ' ' + esc(r.groep) : ''}`).join(' · ') || 'Ouder', rechts: contact(p), act: 'rollenPersoon', attrs: `data-id="${p.id}"` })).join('') || h.leeg('Niemand gevonden')}</div>`;
        }
  ...
  CC.views.afgedaan = (S) => ({ titel: 'Afgedane signalen', html: `<p class="zacht klein">Signalen die trainers of teamleiders hebben afgedaan als "geen actie nodig". Ze komen vanzelf terug als het erger wordt.</p>
    <div class="lijst">${M.afgedaanRecent(S, S.teams.map((t) => t.id), 60).slice().reverse().map((x) => h.rij({ ic: 'check', titel: esc(x.tekst), sub: `${esc(CC.tn(x.teamId))} · ${esc((M.persoon(S, x.door) || { naam: '' }).naam)} · ${D.tijdstip(x.tijd)}${x.notitie ? '<br>' + esc(x.notitie) : ''}`, act: x.spelerId ? 'open' : '', attrs: x.spelerId ? `data-view="speler" data-id="${x.spelerId}"` : '' })).join('') || h.leeg('Niets afgedaan')}</div>` });
      ...
      regels(S) {
        ...
        return `<form data-submit="regelsOk" class="codeform">
          ...
          ${(() => { const sa = i.speeltijdAfwijken || { breedte: false, selectie: true }; return `${h.sectie('Speeltijd')}<div class="kaartje"><p class="klein">Iedereen krijgt eerlijk speeltijd (percentage in bijgewoonde wedstrijden). Mag de trainer iemand een blok minder geven, bijvoorbeeld na weinig trainen?</p><label class="schakel"><span>Breedteteams</span><input type="checkbox" name="sab" ${sa.breedte ? 'checked' : ''}><i></i></label><label class="schakel"><span>Selectieteams</span><input type="checkbox" name="sas" ${sa.selectie ? 'checked' : ''}><i></i></label>
            <p class="klein">Uitslag na de wedstrijd naar de ouders sturen (de trainer of timekeeper kan het per wedstrijd uitzetten):</p>
            <label class="schakel"><span>O7–O10 (KNVB: geen uitslagen publiceren)</span><input type="checkbox" name="uo" ${(i.uitslag || {}).onder ? 'checked' : ''}></label>
            <label class="schakel"><span>O11 en ouder</span><input type="checkbox" name="ub" ${(i.uitslag || {}).boven === false ? '' : 'checked'}></label>
            <label class="schakel"><span>Noem wie er scoorden</span><input type="checkbox" name="um" ${(i.uitslag || {}).makers === false ? '' : 'checked'}></label>
            <p class="klein">Wisselen om de … minuten (per leeftijd). Advies: per blok. De trainer kan per wedstrijd afwijken.</p>
  ...
  CC.on('regelsOk', (f) => {
    ...
    Object.assign(i, { deadlineTraining: n('dt'), deadlineWedstrijd: n('dw'), waarschuwingen: { breedte: n('wb'), selectie: n('ws') }, telaat: { breedte: { kort: n('tkb'), seizoen: n('tsb') }, selectie: { kort: n('tks'), seizoen: n('tss') } }, oproepDagen: n('op'), homeDagen: n('hd'), opnemenUur: n('ou'), speeltijdAfwijken: { breedte: f.sab.checked, selectie: f.sas.checked }, uitslag: { onder: !!(f.uo && f.uo.checked), boven: !f.ub || f.ub.checked, makers: !f.um || f.um.checked }, wissel: Object.fromEntries(CC.SPEELVORMEN.map(([k, , advies, duur]) => { const v = Number(String(f['w' + k].value).replace(',', '.')); return [k, v >= 3 && v <= duur ? v : advies]; })), waarschuwing: f.w.value, trainer: { deadlineUur: n('tu'), drempel: n('tp'), maxSeizoen: n('ts') }, zones: { breedte: { groen: n('bg'), oranje: n('bo') }, selectie: { groen: n('sg'), oranje: n('so') } } });
