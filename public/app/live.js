...
(function () {
  ...
  CC.loginScherm = () => {
    ...
    // Inloggen alleen met de code (Besluit 52): een knop in de mail opent de browser, niet de app op het beginscherm.
    if (L.stap === 'code') return scherm(`<h1>Check je mail</h1><p class="zacht">We hebben een mail gestuurd naar <b>${esc(L.email)}</b>. Typ de code van 6 cijfers uit de mail hieronder over.</p>${foutRegel()}${(() => { const w = store.get(WACHT); return w ? `<div class="info oranje">${icon('info')}<span>Je aanmelding${w.voor ? ` voor <b>${esc(w.voor)}</b>` : ''} is <b>nog niet verstuurd</b>. Dat gebeurt pas als je de code hebt ingevuld.</span></div>` : ''; })()}
