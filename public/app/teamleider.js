...
(function () {
    ...
    schermen: {
      home(S) {
        ...
        return `<article class="kaartje hoofd">
          ...
          ${h.actieBlok(acties, 'aanmeldingen om goed te keuren, nieuwe berichten of taken en vervoer die nog niet rond zijn')}`;
      },
      ...
      team(S) {
        ...
        return `${open.length ? `${h.sectie(`Aanmeldingen goedkeuren (${open.length})`)}<div class="lijst">${open.map((x) => CC.aanmRij(S, x)).join('')}</div>` : ''}
          ...
          ${(() => { const volgt = volgtSpelers(tid); const F = CC.spelerFilter(S, tid, !volgt); return F.bar + `<div class="lijst">${F.lijst.map(({ pl, st, vs, z }) => { const o = pl.ouders.map((x) => M.persoon(S, x)); return h.rij({ ic: h.avatar(pl.voornaam), titel: esc(M.naam(S, pl)), sub: `${volgt ? `${st.pct == null ? '–' : st.pct + '%'} aanwezig · ` : ''}${vs && vs.code !== 'verwacht' ? `${h.chip(vs)} · ` : ''}${o.map((p) => esc(p.naam.split(' ')[0])).join(' en ')}`, rechts: o[0] && o[0].tel ? `<a class="icoonknop groen" href="https://wa.me/${CC.waNummer(o[0].tel)}" target="_blank" rel="noopener" aria-label="WhatsApp ouder van ${esc(pl.voornaam)}" data-stop="1">${icon('message-circle')}</a>` : `<button class="icoonknop uit" data-act="geenTel" data-naam="${esc(o[0] ? o[0].naam : '')}" data-stop="1" aria-label="Nog geen telefoonnummer">${icon('message-circle')}</button>`, act: 'open', attrs: `data-view="speler" data-id="${pl.id}"` }); }).join('') || h.leeg('Nog geen spelers')}</div>`; })()}
  ...
  CC.on('wedstrijdToevoegenOk', (f) => {
    ...
    S.acts.push({ id: 'a' + Date.now(), teamId: tid, soort: 'wedstrijd', datum: f.datum.value, tijd: f.tijd.value, eind: CC.plusMin(f.tijd.value, CC.categorie(M.team(S, tid).cat).duur + 15), thuis, tegen: f.tegen.value, veld: thuis ? 'Veld 1' : '', adres: thuis ? (S.club.sportpark || '') : '', verzamel: `${String(Math.floor(vz / 60)).padStart(2, '0')}:${String(vz % 60).padStart(2, '0')}`, tenue: thuis ? 'Thuistenue (wit/blauw)' : 'Uittenue (blauw)', begeleiderId: M.team(S, tid).teamleiderId, uitslag: null, afgelast: false });
