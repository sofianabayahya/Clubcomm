...
(function () {
  ...
  CC.views.trainerDetail = (S, p) => {
    ...
    return { titel: tr.naam, html: `
      ...
      ${h.sectie('Geschiedenis')}<div class="lijst compact">${t.alles.slice().reverse().map((x) => h.rij({ ic: x.soort === 'niet' ? 'ban' : x.soort === 'telaat' ? 'clock' : 'check', titel: `${D.kort(x.datum)} · ${LABEL[x.soort]}`, sub: `${esc(x.teamId)}${x.reden ? ' · ' + esc(x.reden) : ''}${x.afgelast ? ' · training afgelast' : x.vervanger ? ' · vervanger gevonden' : ''}`, rechts: PUNTEN[x.soort] ? `<span class="chip ${x.soort === 'niet' ? 'rood' : 'oranje'} mini">${PUNTEN[x.soort]} ${PUNTEN[x.soort] === 1 ? 'punt' : 'punten'}</span>` : '' })).join('') || '<p class="zacht klein">Niets geregistreerd.</p>'}</div>
