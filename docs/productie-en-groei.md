...
> *Vóór de ouders erbij komen:* ~~database leegmaken en O12 talententeam inrichten~~ ✅ 25-09 (Besluit 38) → ~~wisselschema instelbaar en zelf aan te passen~~ ✅ 25-09 (Besluit 39) → domein mijnclubcomm.nl koppelen (Vercel klaar, DNS bij Hostnet ingesteld; daarna APP_URL en Supabase-inlog-URL aanpassen) → kind van de initiatiefnemer in het team zetten (rol ouder) → eerste teamleider: appbericht + handleiding teamleider, aanmelden via teamuitnodiging, rol teamleider geven (Teams → Staf) → privacycontact invullen (Regels → Privacy) → toestemming bestuur (presentatie klaar: `docs/presentaties/ClubComm-bestuur.pptx`) → teksten voor teamleiders en ouders (handleidingen klaar: `docs/presentaties/`, nog: appbericht aan de eerste teamleider) → testen op echte telefoons.
